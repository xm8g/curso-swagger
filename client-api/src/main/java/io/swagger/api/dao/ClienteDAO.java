...
public class ClienteDAO {
	...
	public Cliente salva(Cliente cliente) {
		...
		cliente.setDataAtualizacaoRegistro(OffsetDateTime.now());
		cliente.status(true);
	...
	public List<Cliente> consultaPorSobrenome(String sobrenome) {
		...
		for(Cliente clientelz: clientes) {
			if(clientelz.getSobrenome() != null && clientelz.getSobrenome().contains(sobrenome)) {
				idToRead = clientes.indexOf(clientelz);
			}
	...
	public Cliente altera(Cliente cliente) {
		...
		if(idToUpdate != null) {
			...
			clientel.setDataAtualizacaoRegistro(OffsetDateTime.now());
